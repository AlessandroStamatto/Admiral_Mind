...
std::ostream& operator<<(std::ostream& cout, Ship ship) {
    std::string name;
    switch (ship.name) {
        case ShipID::Carrier    : name = "Carrier"; break;
        case ShipID::Battleship : name = "Battleship"; break;
        case ShipID::Submarine  : name = "Submarine"; break;
        case ShipID::Cruiser    : name = "Cruiser"; break;
        case ShipID::Destroyer  : name = "Destroyer"; break;
        case ShipID::Mine       : name = "Mine"; break;
        case ShipID::Island     : name = "Island"; break;
    }
    cout << name << " (" << ship.positions.size() << ") ";
    if (ship.vertical()) {
        if (ship.positions.size())
            cout << "Coluna " << ship.positions[0].j << " :\n\t";
        for (auto& pos: ship.positions) cout << pos.i << " ";
    } else {
        if (ship.positions.size())
            cout << "Linha " << ship.positions[0].i << " :\n\t";
        for (auto& pos: ship.positions) cout << pos.j << " ";
    }
    cout << "\n";
    return cout;
}
...
void centerOrigin(sf::RectangleShape& shape)
{
	...
	shape.setPosition(std::floor(bounds.width / 2.f), std::floor(bounds.height / 2.f));
}
...
void centerOrigin(sf::Sprite& sprite)
{
	...
	sprite.setPosition(std::floor(bounds.width / 2.f), std::floor(bounds.height / 2.f));
}
